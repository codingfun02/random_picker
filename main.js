const pickerForm = document.querySelector(".js-pickerForm"),
  ...
  pickerResetBtn = document.querySelector(".js-resetPicker");
...
const handlePickerResetBtnClick = () => {
  startNumber = 0;
  endNumber = 0;
  randomNumbers = [];
  startNumberInput.value = "";
  endNumberInput.value = "";
  remainingNumbers.innerText = "남은 숫자:";
  pickBtn.innerText = "뽑기";
  pickerForm.removeEventListener("submit", handlePickAgainFormSubmit);
  pickerForm.addEventListener("submit", handlePickerFormSubmit);
};

pickerForm.addEventListener("submit", handlePickerFormSubmit);
pickerResetBtn.addEventListener("click", handlePickerResetBtnClick);
